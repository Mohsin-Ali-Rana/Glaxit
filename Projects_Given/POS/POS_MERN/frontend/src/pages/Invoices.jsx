...
import { FileText, Eye, Printer, RotateCcw, X, AlertTriangle, Trash2 } from 'lucide-react';
...
const Invoices = () => {
    ...
    const handleDeleteInvoice = async (invoiceId) => {
        ...
        try {
            const response = await axios.delete(`/pos/invoices/${invoiceId}`);
            if (response.data.status === 'success') {
                fetchInvoices(pagination.current_page);
            ...
            render: (val, row) => (
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="p-1.5 border border-slate-200 dark:border-slate-800 hover:bg-rose-50 hover:border-rose-200 dark:hover:bg-rose-950/20 rounded-lg text-slate-500 hover:text-rose-600 transition"
